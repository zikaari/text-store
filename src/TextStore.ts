...
export enum TextStoreEvent {
    Insert,
    Remove,
    Replace,
}

export interface ITextStoreInsertEvent {
    changeType: TextStoreEvent.Insert
    range: ITextRange
    text: string
}
export interface ITextStoreReplaceEvent {
    changeType: TextStoreEvent.Replace
    range: ITextRange
    oldText: string
    newText: string
}
export interface ITextStoreRemoveEvent {
    changeType: TextStoreEvent.Remove
    range: ITextRange
    oldText: string
}

export type TextStoreChange = ITextStoreInsertEvent | ITextStoreReplaceEvent | ITextStoreRemoveEvent
export type ChangeCallback = (event: TextStoreEvent, info: TextStoreChange) => void
/**
 ...
 */
class TextStore {
    private subscribers: Set<ChangeCallback>
    ...
    public insert(str: string, at: ITextPosition) {
        ...
        const newFragment = updated.split(/\n/)
        const alterRange: ITextRange = {
            end: {
                col: newFragment[newFragment.length - 1].length,
                line: at.line + newFragment.length,
            },
            start: { ...at },
        }
        // Remove old line and insert updated line(s)
        this.lines.splice(pos.line, 1, ...newFragment)
        for (const cb of [...this.subscribers]) {
            cb(TextStoreEvent.Insert, {
                changeType: TextStoreEvent.Insert,
                range: alterRange,
                text: str,
            })
    ...
    public replace(range: ITextRange, str: string) {
        ...
        this.size -= removedText.length
        this.size += updated.length
        if (typeof str === 'string') {
            for (const cb of [...this.subscribers]) {
                cb(TextStoreEvent.Replace, {
                    changeType: TextStoreEvent.Replace,
                    newText: str,
                    oldText: removedText,
                    range,
